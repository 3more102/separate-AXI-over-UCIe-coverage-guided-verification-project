...
def run_experiment(
    *,
    ...
    """Run paired baseline/guided regressions over identical top-level seeds."""
        ...
        pairs.append(
            {
                ...
                "final_coverage_delta": (
                    float(guided["coverage_ratio"]) - float(baseline["coverage_ratio"])
                ),
            }
        )
    ...
    return {
        ...
        "paired_mean_final_coverage_delta": mean(
            pair["final_coverage_delta"] for pair in pairs
        ),
        "paired_runs": pairs,
        "interpretation_note": (
            "This abstract model validates the experiment harness only; it is not evidence "
            "of UCIe-compliant DUT performance. Repeat the same fixed-budget experiment "
            "with measured RTL/UVM coverage before making verification-efficiency claims."
        ),
    }
