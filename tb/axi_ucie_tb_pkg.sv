package axi_ucie_tb_pkg;
  ...
  class axi_txn extends uvm_sequence_item;
    ...
    // Legal AXI transfer sizes from one byte through the full data-bus width.
  ...
  class axi_coverage extends uvm_subscriber #(axi_txn);
    ...
    function automatic bit [AXI_STRB_W-1:0] legal_strobe_mask(
      ...
      for (int unsigned byte_idx = 0; byte_idx < bytes_per_beat; byte_idx++)
        if ((first_lane + byte_idx) < AXI_STRB_W)
    ...
    function void write(axi_txn t);
      ...
      if (t.kind == AXI_WRITE) begin
        foreach (t.strb_q[i]) begin
          expected_mask = legal_strobe_mask(beat_addr, t.size);
    ...
    task body();
      ...
      repeat (count) begin
        ...
        if (!tr.randomize() with {
          ...
          if (bias_partial) {
            kind == AXI_WRITE;
            size inside {[1:AXI_FULL_SIZE]};
          }
          if (!bias_partial && bias_read && !bias_write) kind == AXI_READ;
          if (!bias_partial && bias_write && !bias_read) kind == AXI_WRITE;
          if (bias_narrow && !bias_full) size inside {[0:AXI_FULL_SIZE-1]};
          if (bias_full && !bias_narrow) size == AXI_FULL_SIZE;
        })
