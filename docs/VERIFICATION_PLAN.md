...
- UVM AXI driver/sequencer, dual monitors, end-to-end transport scoreboard, byte-accurate memory semantics checker, covergroups, coverage-guided sequence, and a directed INCR/FIXED burst semantics test running against a separate burst-capable channel-tunnel reference DUT.
...
| UVM methodology path | Burst-capable reference tunnel + transport scoreboard + memory semantics checker | Connect to packetized bridge/link agent |
