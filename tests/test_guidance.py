...
class GuidanceTests(unittest.TestCase):
    def test_warmup_visits_every_scenario_once(self):
        planner = CoverageGuidedPlanner(seed=7)
        tracker = CoverageTracker()
        seen = []

        for i in range(len(SCENARIO_KINDS)):
            scenario, _ = planner.step(
                tracker,
                seed=100 + i,
                transactions=4,
            )
            seen.append(scenario.kind)

        self.assertEqual(set(seen), set(SCENARIO_KINDS))
    ...
    def test_update_tracks_mean_reward(self):
        planner = CoverageGuidedPlanner(seed=1)
        planner.update("read", 2.0)
        planner.update("read", 4.0)

        self.assertEqual(planner.total_pulls, 2)
        self.assertEqual(planner.arms["read"].pulls, 2)
        self.assertAlmostEqual(planner.arms["read"].mean, 3.0)
