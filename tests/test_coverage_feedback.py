...
class CoverageFeedbackTests(unittest.TestCase):
    def test_object_form_and_bias_mapping(self):
        bins = coverage_feedback.normalize_bins(
            {"bins": {"len.long": 0, "kind.read": 4, "burst.fixed": 0, "size.narrow": 0}}
        )
        result = coverage_feedback.build_bias(bins)
        self.assertEqual(result["uncovered_bins"], ["burst.fixed", "len.long", "size.narrow"])
        self.assertEqual(result["bias_knobs"], ["BIAS_FIXED", "BIAS_LONG", "BIAS_NARROW"])
        self.assertEqual(result["plusargs"], ["+BIAS_FIXED=1", "+BIAS_LONG=1", "+BIAS_NARROW=1"])
